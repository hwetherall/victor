...
>
> **Session 1 closed 2026-05-07 with 13 of 31 stories complete. The "Session 1 status & STORY-017 pickup" section below is the seed for the next session — read it first.**
...
## Pickup: STORY-017 (Researcher) — start here next session

The Researcher is the second agent class. STORY-017 registers it and attaches it to the Investigator's `multiagent.agents` roster so the Investigator can delegate web research mid-investigation. **This is the V2 demo's "spawned a Researcher for margin benchmarks" moment.**

### Critical context: how Anthropic multi-agent actually works

Confirmed via day-1 spike + live test 8 + the docs at `https://platform.claude.com/docs/en/managed-agents/multi-agent`:

- Multi-agent is configured at **agent definition** time via the `multiagent` field on `client.beta.agents.create`. The coordinator (Investigator) declares a roster: `multiagent: { type: "coordinator", agents: [{ type: "agent", id: researcherAgentId }] }`.
- Roster constraints: max 20 unique agents, max 25 concurrent threads at runtime, depth = 1 (coordinator can't have a coordinator-of-coordinators).
- All agents in a multi-agent session **share the same container/filesystem**. A Researcher writing `/tmp/research.md` is readable by the Investigator's `read` tool.
- The session-level event stream is the **primary thread**. Sub-agent activity is summarised here via `session.thread_*` events; full sub-agent activity lives on its own thread (`/v1/sessions/:id/threads/:thread_id/events/stream`).
- Sub-agents can have their own custom tools and skills. The Researcher's custom tool is **Tavily web search** (or just rely on the built-in `web_search`/`web_fetch` from `agent_toolset_20260401` — see the Tavily decision below).

### Acceptance criteria (from spec-v2.md §3.2 and the existing plan)

- [ ] Researcher agent definition file at `agents/managed/researcher/AGENT.md` containing the system prompt from spec-v2.md §3.2 (OODA loop, three stopping criteria).
- [ ] Outcomes rubric for the Researcher at `agents/managed/researcher/RUBRIC.md`: "answer cites at least one source; `stoppedBecause` is one of the three valid options; `searchPath` shows query diversity (no two queries share >3 consecutive words)." LEGAL-003 makes citation-population a HARD pass condition.
- [ ] `scripts/register-researcher.ts` modeled on `scripts/register-bottoms-up-skill.ts` and `scripts/register-investigator.ts`. Outputs `RESEARCHER_AGENT_ID` for `.env.local`.
- [ ] Re-register Investigator with the Researcher in its `multiagent.agents` roster (use the existing `register-investigator.ts` — extend `readMultiagentFromEnv()` similarly to how `readSkillsFromEnv()` works).
- [ ] `lib/managed-agents-client.ts` `createResearcherSession()` stub gets a real implementation. Reuse `consumeStreamEvents` and `dispatchToolResponses` (extracted in session 1's guardian C2 fix). Stopping criteria: `confidenceTarget` 0.8, `maxSearches` 10, `diminishingReturnsThreshold` 3.
- [ ] **Live test 8 / SH4.2 integration re-run:** confirm the Investigator delegates to the Researcher via the `multiagent` roster, the Researcher returns a structured `ResearcherOutput` with at least one citation, and a separate `reasoning_traces` row with `agent_type='researcher'` is persisted (extends STORY-007 — add a code branch in `dispatchV2Leaf` or `persistInvestigatorOutput` to handle Researcher sub-traces).
- [ ] Citations rendered in the OODA timeline (`OODATimeline.tsx` already discriminates `agent_type='researcher'` and shows a violet badge; add a citations sub-section).

### Concrete first-step list (do these in order)

1. **Read first** (no editing): `agents/managed/investigator/AGENT.md`, `scripts/register-investigator.ts`, `lib/managed-agents-client.ts` (especially `consumeStreamEvents` + `dispatchToolResponses` — the Researcher reuses these), `docs/spike-day-1-report.md` (multi-agent section). 15 minutes.
2. **Author** `agents/managed/researcher/AGENT.md` and `RUBRIC.md`. The system prompt skeleton from `spec-v2.md §3.2` is the starting point; calibrate for tightness via the same OODA structure as Investigator. Citation requirement is non-negotiable in both prompt and rubric.
3. **Decide on Tavily.** Built-in `web_fetch` + `web_search` are part of `agent_toolset_20260401`. The Researcher might not need Tavily at all. Spec-v2.md §3.2 calls for Tavily; LEGAL-003 flagged the commercial license question. Recommend: **start with built-in `web_search` only** (no Tavily). Add Tavily as a custom tool later if quality requires it. Saves a license-review cycle and one custom-tool surface.
4. **Write** `scripts/register-researcher.ts`. Pattern: copy `register-investigator.ts`, swap model to `claude-sonnet-4-6` (Sonnet, not Opus — per spec §3.2: faster + cheaper for compressed research), drop the Investigator's custom tools, keep `agent_toolset_20260401`.
5. **Run the registration.** Persist `RESEARCHER_AGENT_ID` to `.env.local`.
6. **Re-register the Investigator** with the Researcher in `multiagent.agents`. The pattern is in `multi-agent.md` doc fetch (in the Anthropic Console docs). Note: this creates a new Investigator agent ID — update `.env.local`.
7. **Restart the Next dev server** (the env var change requires it — see gotcha #7 above).
8. **Re-run** `curl -X POST http://localhost:3000/api/dev/integration-sh42 --max-time 1500 -o /tmp/sh42-researcher.json -w "HTTP %{http_code} | %{time_total}s | %{size_download}B\n"`. Inspect for `session.thread_created` events and Researcher sub-thread activity.
9. **Extend `lib/v2-persistence.ts`** to write Researcher sub-thread reasoning traces under the same `node_id` with `agent_type='researcher'`. (STORY-019-R territory but tightly coupled.)

### Pitfalls specific to STORY-017

- **`session.thread_*` events are different from `session.status_*`.** The existing stream consumer doesn't handle `session.thread_status_idle` — when a sub-agent goes idle for tool confirmation, it cross-posts to the primary thread but with a different event type. Read the multi-agent doc carefully before extending `consumeStreamEvents`.
- **Roster vs runtime spawn.** The agent decides whether/when to spawn sub-agents. You can't force the Investigator to call the Researcher — the Investigator's system prompt has to mention "delegate to a Researcher when...". Update Investigator's `AGENT.md` to explicitly call out Researcher delegation as a tool.
- **Sub-agent token cost.** Spec §10 noted "~15× tokens vs single chat" for multi-agent. Budget for STORY-022 (full ABB run with Researcher) needs revisiting.

### Cost expectations for STORY-017

- Registration: ~zero (skill + agent create are free; just an HTTP call).
- One live SH4.2 re-run with Researcher: ~$2–10 (longer than Investigator-alone because of sub-thread spawning).
- If you iterate the Researcher prompt 2–3 times: budget $10–30 total.

### Stories beyond STORY-017 (in priority order for Daniel demo on day ~18)

| Story | Why it's next-after | Cost impact |
|---|---|---|
| STORY-018 | Test Researcher standalone + via spawn | ~$5 |
| STORY-019-R | Persist Researcher sub-traces | $0 (code) |
| STORY-020 | Calibrate Investigator rubric (the right home for the artifact-upload prompt fix from session 1) | $5–15 (10-leaf eval) |
| STORY-021 | Calibrate Researcher rubric | $2–5 |
| STORY-022 | **Full ABB V2 run.** This is where artifact-upload reliability gets dialled in across 11 leaves. | $30–270 |
| STORY-023 | Pre-bake the demo run | $30–270 |
...
If we skip the cuttable items and land everything else, the demo is plausible by day 18 with one full-ABB pre-bake on day 13–14.
...
### STORY-001: Day-1 spike — exercise every assumption the rest of the plan rests on
...
### STORY-002b: Trace ID propagation through sessions
...
### STORY-004b: Session cost telemetry
