...
## Epic 7: Compliance & Disclosure (ID: EPIC-MICKY-LEGAL)

**User Value:** Micky is safe to demo to an external partner. The "MB" persona is clearly labelled as AI-generated; the memo carries an explicit not-advice disclaimer; named third-party bias commentary does not leak into shareable exports; the user can delete a Micky attempt without nuking the underlying run.

**Dependencies:** STORY-MICKY-2.1 (schema + `micky_runs` table). STORY-MICKY-LEGAL-2 additionally needs STORY-MICKY-4.5.

**Estimated Total Effort:** ~3–4 engineer-days

> **Why this is its own epic, not a tail.** EU AI Act Art. 50(2) requires AI-generated content to be disclosed before it reaches a natural person who could mistake it for human output. The named external demo converts STORY-MICKY-LEGAL-1 into a hard ship-blocker. The compliance review (`.claude/agents/product-counsel`) found no other blockers — internal use is clean — but the disclosure layer must be in place before showing Micky to anyone outside the build team.

---

### STORY-MICKY-LEGAL-1: AI disclosure layer

- **Type:** Technical Story
- **Statement:** As a user (or a regulator), I need every surface that renders Micky's output to declare that the content is AI-generated, so the partner persona is not mistaken for a human partner. Required before any external demo.
- **Files to modify:**
  - `components/MickyMemo.tsx` (from STORY-MICKY-4.2) — add a non-collapsible header badge `AI-generated · Micky` styled distinctly from the rest of the memo; a single-sentence not-advice footer
  - `components/MickyAnnotation.tsx` (from STORY-MICKY-5.1) — change the prefix from `Micky's note:` to `AI · Micky's note:` on every render surface
  - `scripts/export-run-markdown.ts` (extending STORY-MICKY-4.5) — add YAML front-matter `ai_generated: true` to the entire exported file when a Micky memo is included; add a footer line beneath the memo: `_This memo was generated by an AI system. It is not the work of a human partner._`
- **Acceptance Criteria:**
  - [ ] Memo header renders the `AI-generated · Micky` badge — visible, not in a tooltip, not collapsible
  - [ ] Per-node annotation prefix reads `AI · Micky's note:` everywhere it appears
  - [ ] Markdown export front-matter contains `ai_generated: true` whenever a Micky memo is included
  - [ ] Markdown export footer contains the literal sentence above; smoke test asserts the string is present
  - [ ] Smoke check: `rg "AI-generated" components/MickyMemo.tsx components/MickyAnnotation.tsx scripts/export-run-markdown.ts` returns ≥ 3 matches
- **Complexity:** Small (1–2 days)
- **Dependencies:** STORY-MICKY-4.2, STORY-MICKY-5.1, STORY-MICKY-4.5

---

### STORY-MICKY-LEGAL-2: Export guardrails — bias stripping + not-advice disclaimer

- **Type:** Technical Story
- **Statement:** As a user, I want shareable exports of Micky's memo to omit named-third-party bias labels and to carry a not-advice disclaimer, so a forwarded export does not create defamation or misrepresentation exposure.
- **Files to modify:**
  - `scripts/export-run-markdown.ts` — add a `stripSourceBias: boolean` flag (default `true`); when on, scan every Micky-emitted string for any source title or author from the `sources` table appearing within ~80 chars of bias-vocabulary tokens (`pre-disposed`, `biased`, `de-rated`, `discounted`, `conflict-of-interest`) and replace each match with the anonymised template: `Note: one or more sources was flagged as potentially predisposed; findings from those sources were weighted accordingly.`
  - `scripts/export-run-markdown.ts` — render the not-advice disclaimer as a muted footer line under the memo: `_This AI-generated memo is decision support only and does not constitute professional consulting, legal, or financial advice. Apply independent judgment before acting._`
  - `components/MickyMemo.tsx` — render the same not-advice disclaimer as a small footnote below the sign-off (visible in the in-app surface as well)
- **Acceptance Criteria:**
  - [ ] `stripSourceBias` defaults to `true`; can be opted out with `--no-strip-bias` CLI flag
  - [ ] Test: a synthetic Micky output containing `"The ABB internal deck is pre-disposed-favourable; H4 evidence de-rated"` produces an export where neither `ABB` nor any other source title appears within 80 chars of the bias vocabulary
  - [ ] Memo UI footer renders the not-advice disclaimer visibly (not hidden in a tooltip)
  - [ ] Markdown export footer contains the disclaimer string; smoke test asserts presence
  - [ ] New smoke test: `scripts/smoke-export-bias-stripping.ts` exits non-zero if a known-named source appears alongside bias vocabulary in the exported file
- **Complexity:** Medium (1 day)
- **Dependencies:** STORY-MICKY-4.5, STORY-MICKY-LEGAL-1

---

### STORY-MICKY-LEGAL-3: Run retention and per-attempt deletion

- **Type:** Technical Story
- **Statement:** As a user, I need to delete a single Micky attempt without deleting the parent run, and I need stale attempts to expire automatically, so the system honours GDPR right-of-erasure obligations on case material that may name identifiable individuals.
- **Files to modify:**
  - `migrations/<timestamp>_micky-runs-deletion.sql` — add `deleted_at timestamptz` column to `micky_runs`; add a trigger or pg_cron job that sets `deleted_at = now()` on attempts whose parent run's `started_at` is > 90 days ago and not been read since (use a simple `last_accessed_at` column on `runs` if absent — otherwise fall back to `started_at`)
  - `lib/schema.ts` — add `deleted_at: string | null` to the `MickyRun` type
  - `app/api/runs/[id]/micky/route.ts` — add `DELETE /api/runs/[id]/micky?attemptId=<id>` handler; soft-delete via `update micky_runs set deleted_at = now() where id = ...`; ignore soft-deleted rows in the GET response
  - `app/case/[id]/memo/page.tsx` — wire a small "delete this attempt" link next to each attempt in the selector (STORY-MICKY-4.3)
- **Acceptance Criteria:**
  - [ ] Migration adds `deleted_at`; existing rows backfilled with `null`
  - [ ] `GET /api/runs/[id]/micky` filters out rows where `deleted_at IS NOT NULL`
  - [ ] `DELETE /api/runs/[id]/micky?attemptId=<id>` returns 200 and the row is no longer surfaced; parent `runs` row untouched
  - [ ] UI confirmation modal before delete; on success, the attempt selector refreshes
  - [ ] DSAR runbook note added inline in the migration file (comment header) and in `docs/test-plan.md` if that file exists: "Micky attempts are in scope for personal-data deletion requests."
  - [ ] Smoke test: `scripts/smoke-micky-deletion.ts` creates an attempt, soft-deletes it, asserts it is absent from the GET response
- **Complexity:** Medium (1–2 days)
- **Dependencies:** STORY-MICKY-2.1, STORY-MICKY-4.3

---

## Recommended Execution Order

This is a literal to-do list for a Claude Code agent or engineer. Execute sequentially unless a step is marked **(parallel OK with [X])**. LEGAL stories are slotted at the earliest point they can run; LEGAL-1 must close before any external demo.
...
5.  STORY-MICKY-2.1     — migrations/<ts>_micky-runs.sql + MickyOutput/MickyRun types in lib/schema.ts
6.  STORY-MICKY-2.2     — Add micky role to lib/llm-client.ts                     (parallel OK with step 5)
7.  STORY-MICKY-2.3     — Author Micky system prompts in agents/micky.ts (reframe + memo)
8.  STORY-MICKY-2.4     — agents/micky.ts: runReframe function
9.  STORY-MICKY-2.5     — agents/micky.ts: runMemoSynthesizer + runMicky
10. STORY-MICKY-LEGAL-3 — micky_runs.deleted_at + DELETE route + soft-delete UI hook (parallel OK with step 11)
11. STORY-MICKY-3.1     — app/api/runs/[id]/micky/route.ts: POST handler
12. STORY-MICKY-3.2     — GET handler + lib/api-client.ts helpers
13. STORY-MICKY-3.3     — scripts/smoke-micky-agent.ts (end-to-end, live creds)
14. STORY-MICKY-4.1     — app/case/[id]/memo/page.tsx: shell + routing             (parallel OK with step 13)
15. STORY-MICKY-4.2     — components/MickyMemo.tsx: full memo renderer
16. STORY-MICKY-4.3     — Regenerate button + attempt selector in memo page
17. STORY-MICKY-4.4     — Navigation links in KanbanBoard.tsx + app/case/[id]/page.tsx
18. STORY-MICKY-4.5     — Markdown export integration in scripts/export-run-markdown.ts
19. STORY-MICKY-LEGAL-1 — AI disclosure layer (memo + annotation + export front-matter/footer)  ⛔ HARD GATE before any external demo
20. STORY-MICKY-LEGAL-2 — Export guardrails: bias stripping + not-advice disclaimer
21. STORY-MICKY-5.1     — components/MickyAnnotation.tsx                           (parallel OK with steps 17–20)
22. STORY-MICKY-5.2     — Wire MickyAnnotation into HypothesisDrilldown.tsx
23. [SHIP-IF-TIME] STORY-MICKY-6.1 — components/GhostTree.tsx
24. [SHIP-IF-TIME] STORY-MICKY-6.2 — Ghost tree toggle in HypothesisDrilldown.tsx
```
...
| External demo happens before STORY-MICKY-LEGAL-1 lands → AI persona reads as a human partner | High | STORY-MICKY-LEGAL-1 | Ship-blocker; treat as the regulatory gate before any external surface. Re-check before every external demo. |
| Bias-stripping regex misses a synonym (e.g. `"slanted"`, `"motivated"`) | Medium | STORY-MICKY-LEGAL-2 | Smoke test enumerates a fixture set; add new vocabulary tokens as they are observed in real Micky runs |
| GDPR DSAR arrives but `micky_runs.deleted_at` is not yet wired | Medium | STORY-MICKY-LEGAL-3 | LEGAL-3 sequenced at step 10 — directly after the table lands, before the API surface ships, so deletion is always available before a memo is in the wild |
| Markdown export crashes when Micky has never been run for a case | Low | STORY-MICKY-4.5 | Acceptance criterion explicitly requires graceful absence; smoke test asserts both the present and absent paths render |

**Critical path:** STORY-MICKY-1.1 → 2.1 → 2.3 → 2.5 → 3.1 → 4.1 → 4.2 → 4.5 → LEGAL-1. This chain is 9 stories deep and ends at the AI-disclosure gate; everything past it (LEGAL-2, EPIC-MICKY-5, EPIC-MICKY-6) is off the critical path for the external demo.

**Failure isolation checkpoint:** Confirm that `agents/orchestrator.ts` has **zero imports from `agents/micky.ts`** before closing STORY-MICKY-2.5 — and re-confirm after every story that touches the orchestrator. This is the single most important architectural invariant for Micky — if it is violated, a Micky bug will block the entire pipeline.

**External-demo readiness checklist** (run before sharing the memo with anyone outside the build team):
- [ ] STORY-MICKY-LEGAL-1 closed; `AI-generated · Micky` badge visible on memo + annotations + export
- [ ] STORY-MICKY-LEGAL-2 closed; `stripSourceBias` defaults `true` and smoke test green
- [ ] STORY-MICKY-LEGAL-3 closed; user can delete a single attempt
- [ ] `agents/orchestrator.ts` has zero imports from `agents/micky.ts`
- [ ] All smoke tests in `scripts/smoke-micky-*.ts` and `scripts/smoke-rejection-logging.ts` exit 0
- [ ] `.claude/agents/product-counsel` has reviewed the rendered memo + a sample export
